...
const FacultyProfilePage = ({ faculty, facultyId, onBack, onRefresh }) => {
  ...
  const [sundayAttendanceRecords, setSundayAttendanceRecords] = useState([]);
  ...
  const handleToggleSunday = async (dateISO) => {
    try {
      const targetId = faculty?._id || facultyId || currentUser?._id;
      const res = await leaveAPI.toggleSundayAttendance({
        userId: targetId,
        date: dateISO
      });
      toast.success(res.data.message || 'Sunday attendance updated');
      fetchFacultyFullDetails();
    } catch (error) {
      toast.error(error.response?.data?.message || 'Failed to update Sunday attendance');
    }
  };
  ...
  const fetchFacultyFullDetails = async () => {
    ...
    try {
      ...
      const [leavesRes, tasksRes, attendanceRes, sundayRes, holidaysRes] = await Promise.all([
        leaveAPI.getLeaves(),
        taskAPI.getTasks(),
        leaveAPI.getDailyAttendance(),
        leaveAPI.getSundayAttendance(targetId),
        leaveAPI.getHolidays()
      ]);

      setSundayAttendanceRecords(sundayRes.data || []);
      ...
      // Current month approved leaves count
      ...
      // Last month approved leaves count
      ...
      toast.error('Failed to load faculty profile page');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Build Day-by-Day (1 to 30/31) Date Matrix for Selected Month (Starts on Sunday)
  const buildMonthDateMatrix = () => {
    ...
    for (let day = 1; day <= totalDaysInMonth; day++) {
      ...
      // Check if faculty has approved leave covering this day
      ...
      // Check if day falls under announced official college holiday
      ...
      const sundayRecord = sundayAttendanceRecords.find(r => r.date === dateISO && r.isPresent);
      const isSundayWorked = isSunday && !!sundayRecord;
      ...
      daysArray.push({
        ...
        isHoliday: !!activeHolidayOnDay,
        holidayDetails: activeHolidayOnDay || null
      });
    }
  ...
  return (
    <div className="space-y-6 fade-in pb-16">
      {/* Header Banner with Back Button & Edit Profile */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-r from-slate-900 via-slate-800 to-amber-950 p-6 md:p-8 rounded-3xl text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10 space-y-3">
          <div className="flex items-center gap-3 flex-wrap">
            <button
              onClick={onBack}
              className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded-xl font-bold text-xs flex items-center gap-2 border border-white/10 transition backdrop-blur-md active:scale-95 w-fit"
            >
            ...
            {isSelf && (
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white rounded-xl font-extrabold text-xs flex items-center gap-2 shadow-lg shadow-orange-500/30 transition active:scale-95 border border-white/20"
              >
          ...
          <div className="flex items-center gap-4 pt-1">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-2xl md:text-3xl font-black text-white">{profileData?.name}</h2>
                <span className="px-3 py-0.5 bg-orange-500/20 text-orange-400 text-xs font-extrabold rounded-full border border-orange-500/30 uppercase tracking-wider">
                  {profileData?.role === 'hod' ? 'HOD' : profileData?.role === 'admin' ? 'Admin' : 'Faculty Profile'}
                </span>
              </div>
              <div className="text-xs md:text-sm text-slate-300 flex items-center gap-2 flex-wrap mt-1">
                ...
                <span className="text-slate-500">•</span>
                <span className="flex items-center gap-1.5 font-semibold text-emerald-300 bg-emerald-500/20 px-2.5 py-0.5 rounded-lg border border-emerald-500/30">
        ...
        {/* Live Today Duty Status Box */}
        <div className="bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/10 space-y-2 relative z-10 min-w-[240px]">
      ...
      {/* METRICS & MONTHLY COMPARISON CARDS */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-card rounded-3xl p-5 border border-slate-200/80">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] font-extrabold text-orange-500 uppercase tracking-wider">This Month</span>
          </div>
          <p className="text-2xl md:text-3xl font-black text-slate-900">
            {profileData?.monthlyLeavesTaken} <span className="text-xs font-bold text-slate-400">Days</span>
          </p>
          <p className="text-xs font-semibold text-slate-500 mt-1">Leaves Taken Current Month</p>
        </div>

        {/* LAST MONTH LEAVES (COMPARISON) */}
        <div className="glass-card rounded-3xl p-5 border border-slate-200/80 bg-slate-50/50">
          <div className="flex items-center justify-between mb-2">
            <div className="p-3 bg-indigo-50 text-indigo-600 rounded-2xl">
              <FiClock className="w-6 h-6" />
            </div>
            <span className="text-[10px] font-extrabold text-indigo-500 uppercase tracking-wider">Last Month</span>
          </div>
          <p className="text-2xl md:text-3xl font-black text-indigo-700">
            {profileData?.lastMonthLeavesTaken} <span className="text-xs font-bold text-slate-400">Days</span>
          </p>
          <p className="text-xs font-semibold text-slate-500 mt-1">Leaves Taken Previous Month</p>
        </div>
        ...
        <div className="glass-card rounded-3xl p-5 border border-slate-200/80">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] font-extrabold text-blue-500 uppercase tracking-wider">CL Quota</span>
          </div>
          <p className="text-2xl md:text-3xl font-black text-slate-900">
            {profileData?.quota?.casual?.balance} <span className="text-xs font-bold text-slate-400">/ 12 Left</span>
          </p>
          <p className="text-xs font-semibold text-slate-500 mt-1">{profileData?.quota?.casual?.used} Days Casual Leave Used</p>
        </div>
        ...
        <div className="glass-card rounded-3xl p-5 border border-slate-200/80">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] font-extrabold text-rose-500 uppercase tracking-wider">SL Quota</span>
          </div>
          <p className="text-2xl md:text-3xl font-black text-slate-900">
            {profileData?.quota?.sick?.balance} <span className="text-xs font-bold text-slate-400">/ 10 Left</span>
          </p>
          <p className="text-xs font-semibold text-slate-500 mt-1">{profileData?.quota?.sick?.used} Days Sick Leave Used</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-100 space-y-6">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-200 pb-4">
          <div className="flex border-b sm:border-b-0 border-slate-200 gap-4 overflow-x-auto w-full sm:w-auto">
            <button
              type="button"
              onClick={() => setActiveTab('calendar')}
              className={`pb-2.5 font-extrabold text-sm flex items-center gap-2 border-b-2 transition whitespace-nowrap ${
        ...
        {activeTab === 'calendar' && (
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs font-bold text-slate-600 bg-white p-3 sm:p-3.5 rounded-2xl border border-slate-200/80 shadow-xs">
              ...
              <span className="flex items-center gap-1.5">
                <span className="w-3.5 h-3.5 rounded-md bg-purple-600 inline-block shadow-xs"></span>
                <span className="hidden sm:inline">🎉 Official College Holiday</span>
            ...
            <div className="grid grid-cols-7 gap-1 sm:gap-2">
              ...
              {currentMonthMatrix.daysArray.map((day) => {
                ...
                if (day.isLeave) {
                  ...
                  cardStyle = "bg-purple-50 border-purple-300 text-purple-900 shadow-xs ring-1 ring-purple-300/50";
                  statusContent = (
                    <div>
                      <div className="hidden sm:block">
                        <span className="px-2 py-0.5 bg-purple-600 text-white rounded-md text-[10px] font-black uppercase inline-block mb-1 shadow-2xs">
                          🎉 Holiday
                        </span>
                        <p className="text-[11px] font-extrabold truncate text-purple-900" title={day.holidayDetails?.title}>
                          {day.holidayDetails?.title}
                        </p>
                      </div>
                      <div className="sm:hidden">
                        <span className="text-[11px] font-black text-white bg-purple-600 px-1 py-0.5 rounded-md inline-block shadow-2xs" title={day.holidayDetails?.title}>
                          🎉
                        </span>
                      </div>
                    </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-0.5 sm:mb-1">
                      ...
                      {/* Small Rounded Toggle Switch Button for Sunday */}
                      {day.isSunday && !day.isLeave && (
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
        ...
        {/* TAB 2: COMPLETE LEAVE RECORDS & APPROVALS */}
            ...
            <div className="flex items-center justify-between">
              <div className="flex bg-slate-100 p-1 rounded-2xl text-xs font-bold">
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl transition ${statusFilter === 'all' ? 'bg-white text-orange-600 shadow-xs' : 'text-slate-600'}`}
                >
                  All ({facultyLeaves.length})
                </button>
                <button
                  onClick={() => setStatusFilter('pending')}
                  className={`px-3.5 py-1.5 rounded-xl transition ${statusFilter === 'pending' ? 'bg-white text-amber-600 shadow-xs' : 'text-slate-600'}`}
                >
                  Pending ({facultyLeaves.filter(l => l.status === 'pending').length})
                </button>
                <button
                  onClick={() => setStatusFilter('approved')}
                  className={`px-3.5 py-1.5 rounded-xl transition ${statusFilter === 'approved' ? 'bg-white text-emerald-600 shadow-xs' : 'text-slate-600'}`}
                >
                  Approved ({facultyLeaves.filter(l => l.status === 'approved').length})
                </button>
                <button
                  onClick={() => setStatusFilter('rejected')}
                  className={`px-3.5 py-1.5 rounded-xl transition ${statusFilter === 'rejected' ? 'bg-white text-rose-600 shadow-xs' : 'text-slate-600'}`}
                >
            ...
            <div className="overflow-x-auto">
              {filteredLeaves.length === 0 ? (
                <div className="bg-slate-50 rounded-2xl p-12 text-center text-slate-400">
                  ...
                  <p className="text-xs font-bold text-slate-700">No Leave History Records Found</p>
                </div>
              ) : (
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-100 text-[11px] uppercase tracking-wider font-extrabold text-slate-400 bg-slate-50/50">
                      ...
                      <th className="p-3.5 text-right rounded-r-2xl">Actions / Approver Comment</th>
                    </tr>
                  </thead>
                    ...
                    {filteredLeaves.map((leave) => (
                      <tr key={leave._id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="p-3.5 text-right">
                          ...
                          ) : (((leave.applicant?._id === user?._id) || (leave.applicant === user?._id) || (leave.applicant?.email === user?.email) || (profileData?._id === user?._id)) && (leave.status === 'pending' || leave.status === 'approved')) ? (
                            <button
        ...
        {/* TAB 3: FACULTY ASSIGNED TASKS */}
        {activeTab === 'tasks' && (
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-2xl text-center">
                <span className="text-xs font-extrabold uppercase text-indigo-600 block">To Do</span>
                <span className="text-2xl font-black text-indigo-700">{facultyTasks.filter(t => t.status === 'todo').length}</span>
              </div>
              <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl text-center">
                <span className="text-xs font-extrabold uppercase text-amber-600 block">In Progress</span>
                <span className="text-2xl font-black text-amber-700">{facultyTasks.filter(t => t.status === 'inprogress').length}</span>
              </div>
              <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-2xl text-center">
                <span className="text-xs font-extrabold uppercase text-emerald-600 block">Completed</span>
                <span className="text-2xl font-black text-emerald-700">{facultyTasks.filter(t => t.status === 'completed').length}</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {facultyTasks.length === 0 ? (
                <div className="bg-slate-50 rounded-2xl p-12 text-center text-slate-400">
                  ...
                  <p className="text-xs font-bold text-slate-700">No Tasks Assigned To This Faculty Member</p>
                </div>
              ) : (
      ...
      <Modal isOpen={!!selectedMonthDetailsModal} onClose={() => setSelectedMonthDetailsModal(null)}>
