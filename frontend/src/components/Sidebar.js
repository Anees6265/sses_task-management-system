...
const Sidebar = ({ activeView, setActiveView }) => {
  ...
  return (
    <aside className={`bg-white border-r border-gray-200 transition-all duration-300 hidden md:block ${
      isCollapsed ? 'w-16 md:w-20' : 'w-48 md:w-64'
    }`}>
      <div className="p-2 md:p-4">
        <button
          onClick={() => setIsCollapsed(!isCollapsed)}
          className="w-full mb-4 md:mb-6 p-2 hover:bg-gray-100 rounded-lg transition text-gray-600 text-sm md:text-base"
        >
          {isCollapsed ? '→' : '←'}
        </button>
        ...
        {!isCollapsed && (
          <div className="mt-6 md:mt-8 p-3 md:p-4 bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg border border-orange-200">
